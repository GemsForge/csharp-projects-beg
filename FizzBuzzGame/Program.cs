...
namespace FizzBuzz
{
    public class Program
    {
        public static void Main(string[] args)
        {
           ...
           IFizzBuzzService _fbService = new FizzBuzzService();
            ...
            var (fizzes, buzzes, fizzBuzzes) = CountFizzBuzzes(_fbService);

            // Display the results
            Console.WriteLine($"Total Points: {_fbService.TallyPoints()} (Fizz {fizzes}x | Buzz {buzzes}x | FizzBuzz {fizzBuzzes}x)");
        }
        ...
        private static (int fizzes, int buzzes, int fizzBuzzes) CountFizzBuzzes(IFizzBuzzService fbService)
        {
            int fizzes = 0;
            int buzzes = 0;
            int fizzBuzzes = 0;
